...
class SearcherDijkstra(Searcher):
    def __init__(self):
        super(SearcherDijkstra, self).__init__()
        self.pq : List[State]
    ...
    def reset(self) -> None:
        super(SearcherDijkstra, self).reset()
        self.pq = []
        self.prevs = []
        self.min_costs = []
        self.is_init = False
    ...
    def search_per_step(self, maze : Maze) -> bool:
        ...
        print(f"cur ({cur.x}, {cur.y}), cost-from-start {cur.cost_from_start}")
        if (cur.x, cur.y) == maze.end:
            # reach to the targer
        ...
        for next_x, next_y in maze.get_neighbors((cur.x, cur.y)):
            ...
            if next_cost < self.min_costs[next_x][next_y]:
                ...
                heapq.heappush(self.pq, next_state)
